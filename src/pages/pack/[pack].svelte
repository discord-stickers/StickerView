<script>
    ...
    let pack = $params.pack.replace(/\+/g, " ")
    ...
    <script>
        function load(e) {
            ...
            e.onload = ()=>{
                e.src = e.dataset.src;
                e.onload = ()=>{
                    e.parentElement.className += " tooltipped tooltipped-n";
                    e.parentElement.childNodes[0].setAttribute("onClick", "")
                    e.parentElement.childNodes[0].className = e.parentElement.childNodes[0].className.split("tooltipped tooltipped-n")[1]
                    e.parentElement.setAttribute("aria-label", "Click me to copy the URL to your clipboard!");
                    e.parentElement.setAttribute("onClick", `copy(this, "${e.src}")`)
                }
            }
        }
...
<main>
    ...
    <p>currently viewing: {pack} - {format}</p>
    <span onclick="highBandwith()">toggle high bandwith mode</span>
    <br>
    <span onclick="history.back()">go back</span>
    <br>
    <input type="text" placeholder="search inside this pack" bind:value={searchQuery}>

	<View {searchQuery} type="sticker" />
</main>
<style>
    main {
        height: 100%;
